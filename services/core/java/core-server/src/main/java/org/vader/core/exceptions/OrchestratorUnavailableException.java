 ...
 * a transport-level error. Distinct from {@link OrchestratorResponseException}, which means the
 * LLM answered but the answer was unusable. This condition is typically transient (the backend is
 * still warming up), so callers may retry.
 */
