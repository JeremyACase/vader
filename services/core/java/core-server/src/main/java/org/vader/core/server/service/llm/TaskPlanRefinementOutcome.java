 ...
 * LLM was unreachable, for the same reason {@link DecompositionOutcome} isn't: connectivity
 * failure is an expected, everyday outcome the calling strategy makes its own policy decision
 * about, not a bug in the request/response plumbing.
 *
 * @param verdict the critique's verdict, or {@code null} if {@code unreachableReason} is set
 * @param unreachableReason why the LLM could not be reached, or {@code null} on success
 */
public record TaskPlanRefinementOutcome(
    TaskPlanRefinementVerdict verdict, String unreachableReason) {
